export const blogPosts = [
  {
    id: "1",
    title: "The Death of Seed Phrases? Exploring Social Recovery in RAVEX",
    excerpt:
    ...
    content: `# The Death of Seed Phrases? Exploring Social Recovery in RAVEX

The seed phrase, long considered the cornerstone of crypto self-custody, is increasingly becoming a liability in modern blockchain architectures. With the rise of smart contract accounts and programmable wallets, projects like RAVEX are reimagining account recovery by implementing a technically robust framework for social recovery. This article explores the architecture, mechanics, and implications of RAVEX's social recovery system—and how it signals the end of traditional seed phrase reliance.
...
3. **Recovery Module**  
   A dedicated contract that manages initiation, approval, time-locking, and execution of recovery flows. RAVEX includes both native and pluggable modules for developer-defined logic.
...
The lifecycle of a typical social recovery process in RAVEX consists of the following phases:
...
RAVEX's design emphasizes defense-in-depth. Key security features include:
...
| Feature | Seed Phrase | Traditional Custody | RAVEX Social Recovery |
|---------|-------------|---------------------|------------------------|
...
RAVEX delivers a recovery system that retains decentralization while improving resilience, usability, and security.

## Implications for Developers and dApps

Social recovery in RAVEX is not only a user-facing benefit but also a powerful primitive for application developers. dApps can:
...
RAVEX's social recovery model represents a critical step toward eliminating one of the biggest UX and security challenges in crypto: the fragility of seed phrases. By embedding programmable, guardian-based recovery directly into the account architecture, RAVEX introduces a new trust model for decentralized identity and wallet management.
...
This model offers the flexibility of traditional authentication systems without compromising decentralization, custody, or privacy—making it a viable alternative for individuals, institutions, and protocol-level wallet design going forward.`,
  ...
  {
    id: "2",
    title: "Top 10 Use Cases That RAVEX Unlocks for Developers",
    excerpt:
...
**RAVEX's Solution:**
RAVEX enables developers to onboard users via social logins (Google, Apple, etc.) while generating smart accounts on the fly. Combined with Paymaster infrastructure, dApps can fully sponsor transaction fees during onboarding, removing the need for users to hold ETH.
...
**RAVEX's Solution:**
RAVEX SDK allows developers to embed fully programmable smart wallets directly into dApps. These accounts are smart contracts with customizable modules—like token allowlists, throttling, or compliance guards—implemented at the account layer.
...
**RAVEX's Solution:**
Using RAVEXOperation, developers can define a sequence of contract calls that execute atomically in a single batch transaction. This drastically simplifies multi-step workflows on-chain.
...
**RAVEX's Solution:**
RAVEX accounts support social recovery and programmable recovery workflows. Developers can define a recovery policy involving guardians, time delays, or behavioral rules (e.g. inactivity-based recovery).
...
**RAVEX's Solution:**
RAVEX Smart Accounts natively support multi-signature logic and modular RBAC policies, allowing developers to assign granular permissions (e.g., spender, approver, admin) per user or key.
...
**RAVEX's Solution:**
RAVEX Smart Accounts are module-based, where developers can register custom modules that define additional logic executed at the account level. Modules can be swapped, upgraded, or revoked at runtime.
...
**RAVEX's Solution:**
RAVEX supports AI-driven smart accounts, allowing developers to build agents that analyze portfolio state, forecast risks, or optimize yield strategies—and then execute decisions using RAVEXOperation.
...
**RAVEX's Solution:**
RAVEX offers chain-agnostic smart accounts with a unified interface and chain-aware execution logic. Developers can use the same SDK and account contract across supported EVM networks with automatic network detection and routing.
...
RAVEX is not just a wallet SDK; it's an execution environment and account framework that redefines how developers build with Web3 primitives. With modularity, abstraction, and on-chain programmability at its core, RAVEX unlocks use cases that were previously impractical or impossible.

By adopting RAVEX's architecture, developers gain direct access to the next evolution of decentralized app design, where wallets are programmable agents, gas is abstracted, and UX is indistinguishable from Web2.

For developers serious about building for the next billion users, RAVEX is not a feature—it's foundational infrastructure.`,
    author: "RAVEX Team",
    date: "Apr 15, 2025",
    readTime: "6 min",
    category: "Technical",
...
In RAVEX, this is achieved through the RAVEX Smart Account SDK, which exposes hooks to plug in AI logic — either off-chain (via controller agents) or directly into the account modules.
...
RAVEX flips this model: instead of building bots that control wallets, you build wallets that are bots.

## Key Capabilities of AI Smart Accounts in RAVEX
...
RAVEX allows account-level modules to intercept transactions before execution. Developers can embed ML models or heuristics that reject suspicious, non-compliant, or mispriced transactions.
...
## Architecture: How RAVEX Enables AI at the Account Level

### Smart Account Core (Modular Logic)

Each RAVEX Smart Account is an upgradable smart contract that uses a modular architecture:
* Execution modules
* Validation modules
...
AI agents build and sign RAVEXOperations, which are submitted via RAVEX's bundler infrastructure.
...
* Signs and submits to the RAVEX Mempool
...
1. **Deploy RAVEX Smart Account**
   * Via SDK or RAVEX Factory API
...
3. **Run Agent Service**
   ...
   * Generate RAVEXOperations based on AI strategy
   * Submit via bundler
...
* Simulation is vital. RAVEX supports full off-chain simulation of UserOperations before execution, reducing failure risk.

## Conclusion

AI-powered smart accounts aren't just a novel concept — they're the logical next step in wallet evolution. By embedding autonomous behavior, RAVEX allows developers to build wallets that act more like agents: programmable, secure, and intelligent.

As the complexity of on-chain systems grows, the need for automation at the account level will become mandatory. RAVEX provides the infrastructure to build this today.

If you're building autonomous finance, intelligent agents, or advanced DeFi UX — RAVEX's AI smart accounts are your execution layer.`,
    author: "RAVEX Team",
    date: "Nov 28, 2024",
    readTime: "8 min",
    category: "Innovation",
